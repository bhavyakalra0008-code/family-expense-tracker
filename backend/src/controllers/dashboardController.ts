...
type AdminTransaction = {
  ...
  category: string;
  date: Date;
  child: { id: string; name: string; colorTag: string | null };
};
...
export async function getAdminDashboard(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const children: DashboardChild[] = await prisma.child.findMany({
      where: { familyId },
      select: {
        id: true,
        name: true,
    ...
    const currentMonthTransactions: AdminTransaction[] = await prisma.transaction.findMany({
      where: {
        familyId,
        date: {
          gte: startOfMonth,
...
export async function getChildDashboard(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const childTransactions: Prisma.TransactionGetPayload<{}>[] = await prisma.transaction.findMany({
      where: {
        childId,
        date: {
          gte: startOfMonth,
