/* =========================================================
   ...
   AWS / PRODUCTION:
     Set window.LEDGER_API_BASE before this script loads:
       Option A — in index.html (recommended for static hosting):
         <script>window.LEDGER_API_BASE = "https://api.your-domain.com/api";</script>
       Option B — in a separate config.js file included before app.js:
         window.LEDGER_API_BASE = "https://api.your-domain.com/api";

   DO NOT hardcode a production URL here.
   ========================================================= */
const API_BASE = (typeof window !== 'undefined' && window.LEDGER_API_BASE)
  ? window.LEDGER_API_BASE
...
$$('.current-month').forEach((el) => { el.textContent = currentMonthYear(); });
...
function notifyBudgetAlerts(children) {
    const exceeded = children.filter((child) => child.isOverLimit);
    const newAlerts = exceeded.filter((child) => !state.ui.budgetAlertedChildIds.has(child.id));
    if (!newAlerts.length) return;

    newAlerts.forEach((child) => state.ui.budgetAlertedChildIds.add(child.id));
    const message = newAlerts.length === 1
        ? `Budget alert: ${newAlerts[0].name} has exceeded the monthly limit.`
        : `Budget alert: ${newAlerts.map((child) => child.name).join(', ')} exceeded their monthly limits.`;
    toast(message, 'fa-solid fa-triangle-exclamation');
}

function renderBudgetNotifications(children) {
    const notifications = $('#budget-notifications');
    const count = $('#notification-count');
    if (!notifications || !count) return;

    const exceeded = children.filter((child) => child.isOverLimit);
    count.textContent = `${exceeded.length} ${exceeded.length === 1 ? 'alert' : 'alerts'}`;

    if (!exceeded.length) {
        notifications.innerHTML = '<p class="muted small notification-empty"><i class="fa-solid fa-circle-check"></i> No budget alerts. Everyone is within their monthly limit.</p>';
        return;
    }

    notifications.innerHTML = exceeded.map((child) => `
        <div class="budget-notification">
            <span class="notification-icon"><i class="fa-solid fa-triangle-exclamation"></i></span>
            <div class="notification-body">
                <strong>${escapeHtml(child.name)} exceeded the monthly limit</strong>
                <span>${rupee(child.currentMonthSpend)} spent of ${rupee(child.monthlyLimit)}</span>
            </div>
        </div>`).join('');
}
...
function logoutAdmin() {
    ...
    state.ui.role = null;
    state.ui.token = null;
    state.ui.budgetAlertedChildIds.clear();
    localStorage.removeItem('ledger_token');
    showPage('landing');
    toast('Logged out', 'fa-solid fa-right-from-bracket');
}

$('#btn-logout-admin').addEventListener('click', logoutAdmin);
$('#btn-logout-admin-top').addEventListener('click', logoutAdmin);
...
async function renderAvatarGrid() {
    ...
    grid.innerHTML = '';
    const familySelect = $('#child-family-select');
    familySelect.innerHTML = '<option value="">Choose your Family Code</option>';
    familySelect.disabled = true;

    try {
        const res = await apiFetch('/auth/families');
        (res.families || []).forEach((family) => {
            const option = document.createElement('option');
            option.value = family.code;
            option.textContent = `${family.name} (${family.code})`;
            familySelect.appendChild(option);
        });
        familySelect.disabled = false;
        grid.innerHTML = '<p class="muted small">Choose your Family Code to load child profiles.</p>';
    } catch (err) {
        state.children = [];
        grid.innerHTML = '<p class="muted small">Unable to load families from the local backend.</p>';
        return;
    }
}

async function loadChildrenForFamily(familyCode) {
    const grid = $('#avatar-grid');
    grid.innerHTML = '';
    state.children = [];
    state.ui.selectedFamilyCode = familyCode || null;
    ...
    try {
        const res = await apiFetch(`/auth/child/profiles?familyCode=${encodeURIComponent(familyCode)}`);
        ...
        state.children = [];
        grid.innerHTML = '<p class="muted small">Unable to load child profiles from the local backend.</p>';
        return;
    }

    if (!state.children.length) {
        grid.innerHTML = '<p class="muted small">No child profiles exist yet. Register as a parent and add a child first.</p>';
        return;
    }
...
$('#child-family-select').addEventListener('change', (event) => loadChildrenForFamily(event.target.value));
...
$('#form-child-login').addEventListener('submit', async (e) => {
    ...
    try {
        const res = await apiFetch('/auth/child/login', {
            method: 'POST',
            body: JSON.stringify({ familyCode: state.ui.selectedFamilyCode, childId, pin: enteredPin }),
        });
...
function renderAdminDashboardFromApi(dash) {
    ...
    if (dash.family?.code) {
        $('#family-code-display').textContent = `Family Code: ${dash.family.code}`;
    }

    notifyBudgetAlerts(dash.childrenBreakdown);
    renderBudgetNotifications(dash.childrenBreakdown);
    ...
    dash.childrenBreakdown.forEach(c => {
      ...
      <div class="member-spend">
        ...
        <div class="lim">of ${rupee(c.monthlyLimit)}</div>
            ...
            <button class="icon-btn remove-child-btn" data-child-id="${c.id}" title="Remove ${escapeHtml(c.name)}" aria-label="Remove ${escapeHtml(c.name)}">
...
$('#members-list').addEventListener('click', async (event) => {
    const button = event.target.closest('.remove-child-btn');
    if (!button) return;

    const child = childById(button.dataset.childId);
    if (!child) return;

    const confirmed = window.confirm(`Remove ${child.name}? Their linked transactions will also be removed.`);
    if (!confirmed) return;

    try {
        await apiFetch(`/family/children/${child.id}`, { method: 'DELETE' });
        toast(`${child.name} removed`, 'fa-solid fa-user-minus');
        await refreshAdminData();
    } catch (err) {
        toast(err.message || 'Failed to remove child', 'fa-solid fa-triangle-exclamation');
    }
});
...
function renderChildDashboardFromApi(dash) {
    ...
    if (over && !state.ui.budgetAlertedChildIds.has(child.id)) {
        state.ui.budgetAlertedChildIds.add(child.id);
        toast(`Budget alert: ${child.name} has exceeded the monthly limit.`, 'fa-solid fa-triangle-exclamation');
    }
